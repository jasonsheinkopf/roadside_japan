---
title: "Tendo, the Shogi-Piece Town"
summary: "The town that carves about 95% of Japan's shogi (Japanese chess) pieces leans all the way in — giant chess pieces on the streets, shogi-shaped everything, craftsmen you can watch at work, and an April festival played on a board of costumed humans."
prefecture: yamagata
city: "Tendo"
address: "Around Tendo Station, Tendo, Yamagata"
lat: 38.3620
lng: 140.3780
category: oddity
tags: [shogi, craft, oddity, kids, hidden-gem, hands-on]
seasons: []
months: []
seasonNote: "A craft town year-round; the famous Human Shogi festival is held in late April."
difficulty: easy
timeRequired: half-day
cost:
  type: free
  note: "Free to explore; workshops, the museum, and piece-engraving experiences are paid."
parking:
  available: yes
  note: "Parking around the station and onsen district."
transit:
  nearestStation: "Tendo Station"
  note: "Everything clusters around Tendo Station on the Yamagata Shinkansen line."
accessibility:
  wheelchair: partial
  note: "Flat town streets; individual workshops vary."
dogFriendly: limited
heroImage: "/images/attractions/tendo-shogi-town.svg"
photos:
  - src: "/images/attractions/tendo-shogi-town-1.svg"
    alt: "Giant wooden shogi (Japanese chess) pieces standing in a town square"
    caption: "The town that makes 95% of Japan's shogi pieces. Even the manholes are chess."
  - src: "/images/attractions/tendo-shogi-town-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel posing with giant wooden shogi pieces"
    caption: "Cinnamon transformed into a shogi piece and tried to promote himself to king. Denied."
website: "https://bussan-tendo.gr.jp/"
tips:
  - "Try engraving your own shogi piece at a workshop — a great souvenir and kid activity."
  - "Spot the shogi motifs everywhere: pieces on the streets, the manhole covers, the station."
  - "Come in late April for the Human Shogi, a live game played by costumed people on a giant board."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.82
sources:
  - title: "Tendo, Yamagata — Wikipedia"
    url: "https://en.wikipedia.org/wiki/Tend%C5%8D,_Yamagata"
aiSummary: "Tendo in Yamagata produces the vast majority of Japan's shogi pieces and celebrates the craft with shogi-themed streets, workshops, a museum, and an annual Human Shogi festival."
aiKeywords: [tendo, shogi, japanese chess, human shogi, yamagata, craft]
related:
  - omiya-bonsai-village
createdAt: 2026-06-16
updatedAt: 2026-06-16
cinnamon:
  quote: "The town that makes Japan's shogi pieces. Even the manhole covers play. I lost to a manhole cover."
  emoji: "♟️"
  report: |-
    Tendo carves about 95% of Japan's shogi pieces, and it wears the job everywhere: giant pieces standing on street corners, piece-shaped manhole covers, shogi-piece cakes, and workshops where craftsmen ink kanji onto blessed maple with brushes trimmed to a whisper.

    I watched a master write the same character forty times, each one identical and each one alive, which is calligraphy's whole trick. In spring the town holds Ningen Shogi — HUMAN shogi, played with armored people on a giant hillside board under blossoms — which I regrettably missed and about which my journal contains only the words 'RETURN. APRIL.'

    The humans who stop here are mostly shogi pilgrims; the rest of Japan hurries through to the onsens. Stop. Watch the brushes. Buy a keychain piece with your name inked by a national craftsman for pocket change.
  snapshots:
    - cast: [cinnamon]
      prop: "♟️"
      caption: "Watching a master ink the same character forty times, each one identical and each one alive — calligraphy's whole trick, on blessed maple."
    - cast: [cinnamon, mon]
      prop: "🏘️"
      caption: "Losing to a manhole cover. My journal on the spring HUMAN shogi festival — armored people on a hillside board under blossoms — reads, in full: 'RETURN. APRIL.'"
---

If a town can have a hobby, **Tendo's** is **shogi** — Japanese chess. It produces roughly **95% of all the shogi pieces in Japan**, and it celebrates that fact with a wholehearted, slightly silly enthusiasm that makes it a delight to wander.

## Why It's Interesting

The whole town is in on the joke: **giant shogi pieces** stand in squares and parks, the **manhole covers** are chess pieces, and shogi motifs decorate the station and shops. You can watch master craftsmen **engrave pieces** by hand, try carving your own at a workshop, and learn the game at a small museum. The grand finale is the springtime **Human Shogi**, in which a live game is played out on a giant outdoor board by **people in full samurai costume** as the "pieces." It's the kind of single-minded local obsession that's pure Cinnamon Land.

## Best Time to Visit

**Year-round** for the craft and workshops; **late April** for the spectacular **Human Shogi** festival (and the cherry blossoms around it).

## Getting There

Everything clusters around **Tendo Station** on the Yamagata Shinkansen — an easy, fun half-day.
