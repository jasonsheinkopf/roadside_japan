---
title: "Bato Hiroshige Museum of Art"
summary: "A serene, award-winning Kengo Kuma building wrapped head to toe in fine local-cedar latticework, built to hold ukiyo-e by Hiroshige — architecture that turns the artist's famous slanting rain into wood and light."
prefecture: tochigi
city: "Nakagawa"
address: "116-9 Bato, Nakagawa, Nasu District, Tochigi"
lat: 36.7820
lng: 140.1560
category: art
tags: [art, museum, architecture, kengo-kuma, ukiyo-e, hiroshige, indoor]
seasons: []
months: []
difficulty: easy
timeRequired: one-hour
cost:
  type: paid
  priceJpy: 700
  note: "Adult admission; varies with special exhibitions."
parking:
  available: yes
  note: "Free parking on site."
transit:
  nearestStation: "Nasushiobara Station + bus"
  note: "Bus toward Bato/Nakagawa; remote, so a car is easiest."
accessibility:
  wheelchair: yes
  note: "Step-free, modern museum."
dogFriendly: no
heroImage: "https://upload.wikimedia.org/wikipedia/commons/thumb/1/19/Nakagawa-machi_Bato_Hiroshige_Museum_of_Art%2C_main_building.JPG/960px-Nakagawa-machi_Bato_Hiroshige_Museum_of_Art%2C_main_building.JPG"
photos:
  - src: "https://upload.wikimedia.org/wikipedia/commons/thumb/1/19/Nakagawa-machi_Bato_Hiroshige_Museum_of_Art%2C_main_building.JPG/960px-Nakagawa-machi_Bato_Hiroshige_Museum_of_Art%2C_main_building.JPG"
    alt: "Museum main building (Kengo Kuma)"
    credit: "Photo via Wikimedia Commons"
    creditUrl: "https://commons.wikimedia.org/wiki/File:Nakagawa-machi_Bato_Hiroshige_Museum_of_Art%2C_main_building.JPG"
  - src: "/images/attractions/bato-hiroshige-museum-1.svg"
    alt: "A low museum building wrapped in fine cedar latticework with a ukiyo-e wave print inside"
    caption: "A museum dressed in cedar slats by Kengo Kuma, full of Hiroshige's woodblock rain."
  - src: "/images/attractions/bato-hiroshige-museum-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel before a cedar-latticed museum and a ukiyo-e print"
    caption: "Cinnamon tried to store an acorn in the cedar slats. Architecture is not a pantry, Cinnamon."
website: "https://hiroshige.bato.tochigi.jp/"
tips:
  - "Look up and along the eaves — the whole building is a study in repeating cedar louvers."
  - "The collection centers on Hiroshige's woodblock prints and Utagawa-school works."
  - "It's remote; combine it with a drive along the Naka River valley."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.83
sources:
  - title: "Nakagawa-machi Bato Hiroshige Museum of Art — Wikipedia"
    url: "https://en.wikipedia.org/wiki/Nakagawa-machi_Bat%C5%8D_Hiroshige_Museum_of_Art"
  - title: "Kengo Kuma & Associates — project page"
    url: "https://kkaa.co.jp/en/project/nakagawa-machi-bato-hiroshige-museum-of-art/"
aiSummary: "An art museum in Bato, Nakagawa, Tochigi, in an award-winning Kengo Kuma building clad in local cedar louvers, holding ukiyo-e by Hiroshige and Utagawa-school prints."
aiKeywords: [bato hiroshige museum, kengo kuma, ukiyo-e, hiroshige, nakagawa, tochigi, architecture]
related:
  - ns-yard
createdAt: 2026-06-16
updatedAt: 2026-07-13
cinnamon:
  quote: "Hiroshige drew rain so well I shook my fur dry afterward. Reflex."
  emoji: "🌊"
---

In the quiet Naka River country east of Nasu, the small town of **Bato** holds a building that architecture pilgrims cross the country to see: the **Nakagawa-machi Bato Hiroshige Museum of Art**, an early masterpiece by **Kengo Kuma**.

## Why It's Interesting

Kuma wrapped the long, low building **head to toe in slender louvers of local Yamizo cedar**, so the whole structure seems to dissolve into a soft screen of light and shadow — an attempt, he said, to translate the **slanting rain** of Hiroshige's famous woodblock prints into architecture. Inside hangs exactly that: Hiroshige's **ukiyo-e**, hand-painted works, and Utagawa-school prints. The art and the building rhyme so well that the museum became as celebrated as its collection, winning a stack of architecture awards.

## Best Time to Visit

**Year-round** — indoor and contemplative. The cedar facade is beautiful in any light, especially low afternoon sun.

## Getting There

It's **remote**; easiest by **car** along the Naka River valley, with bus connections from Nasushiobara for the determined.
