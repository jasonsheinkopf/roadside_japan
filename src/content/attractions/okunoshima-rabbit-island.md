---
title: "Okunoshima (Rabbit Island)"
summary: "A small island in the Inland Sea overrun by hundreds of friendly wild rabbits — and a sobering secret past as a hidden poison-gas factory the maps once deliberately erased."
prefecture: hiroshima
city: "Takehara"
address: "Okunoshima, Tadanoumi, Takehara, Hiroshima"
lat: 34.3107
lng: 132.9889
category: animal
tags: [animal, kids, outdoor, photography, history, hidden-gem]
seasons: [spring, summer, autumn]
months: []
seasonNote: "Pleasant spring through autumn; the ferry and rabbits are there year-round."
difficulty: easy
timeRequired: full-day
cost:
  type: paid
  note: "Free to roam; you pay for the ferry (and optional rabbit food/bicycle)."
parking:
  available: yes
  note: "Parking at Tadanoumi ferry terminal on the mainland."
transit:
  nearestStation: "Tadanoumi Station (JR Kure Line) + ferry"
  note: "Short walk from Tadanoumi Station to the ferry pier."
accessibility:
  wheelchair: partial
  note: "Ferry and main paths are flat; some museum/trail sections are not."
dogFriendly: no
heroImage: "/images/attractions/okunoshima-rabbit-island.svg"
tips:
  - "Buy rabbit pellets BEFORE the ferry — there's nowhere to buy food on the island."
  - "Don't bring dogs or chase the rabbits; let them come to you."
  - "Visit the Poison Gas Museum to understand the island's hidden wartime history."
status: open
approval: published
source: editorial
submittedBy: editorial
featured: false
aiSummary: "Inland Sea island famous for hundreds of tame wild rabbits, with a dark WWII history as a secret chemical-weapons site."
aiKeywords: [usagi shima, rabbits, inland sea, poison gas, takehara, ferry]
related:
  - nagoro-scarecrow-village
createdAt: 2025-07-22
updatedAt: 2025-09-18
---

A fifteen-minute ferry from the Hiroshima coast lands you on **Okunoshima**, where the welcoming committee is several hundred wild rabbits that will hop straight into your lap at the rustle of a food bag.

## Why It's Interesting

The contrast is unforgettable. The island is a fluffy, joyful free-for-all — and also a place that was literally **erased from maps**. From the 1920s, Japan secretly manufactured chemical weapons here; the ruins of the gas factory still stand, and a small museum tells the difficult story honestly. The rabbits, descended from animals released after the war, now blanket the island.

## Best Time to Visit

Spring and autumn are the most comfortable. Summer is hot but popular with families; the rabbits are active year-round.

## Getting There

Take the JR Kure Line to Tadanoumi, walk to the pier, and hop the ferry. Rent a bicycle on arrival to loop the whole island in an afternoon.
