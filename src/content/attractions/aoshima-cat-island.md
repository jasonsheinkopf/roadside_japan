---
title: "Aoshima Cat Island"
summary: "A tiny fishing islet off Ehime where cats outnumber the handful of human residents many times over — no shops, no hotels, no cars, just a sleepy harbor overrun by friendly felines."
prefecture: ehime
city: "Ozu"
address: "Aoshima, Ozu, Ehime"
lat: 33.8146
lng: 132.6586
category: animal
tags: [cats, island, ferry, animal, oddity, hidden-gem, fishing-village]
seasons: []
months: []
seasonNote: "Visitable year-round, but the ferry is tiny and weather-dependent — calm days only, and it can be cancelled."
difficulty: moderate
timeRequired: half-day
cost:
  type: paid
  note: "Cheap ferry fare from Nagahama; the island itself has no admission — and no shops."
parking:
  available: nearby
  note: "Park near Nagahama Port on the mainland; you leave the car behind to take the ferry."
transit:
  nearestStation: "Iyo-Nagahama Station"
  note: "Train to Iyo-Nagahama, walk to the port, then a short ferry across to the island."
accessibility:
  wheelchair: no
  note: "Small boat, a working harbor, and uneven village paths — not wheelchair friendly."
dogFriendly: no
heroImage: "/images/attractions/aoshima-cat-island.svg"
photos:
  - src: "/images/attractions/aoshima-cat-island-1.svg"
    alt: "A contented cat sprawled asleep on an overturned fishing boat"
  - src: "/images/attractions/aoshima-cat-island-2.svg"
    alt: "A swarm of cats stampeding down the pier toward the arriving ferry"
  - src: "/images/attractions/aoshima-cat-island-3.svg"
    alt: "A single dignified cat sitting atop the harbor wall like the island's mayor"
website: "https://en.wikipedia.org/wiki/Aoshima,_Ehime"
tips:
  - "Bring everything you need — there are no shops, no vending machines, and no toilets to count on. Pack water and snacks."
  - "Don't bring dogs, and feed cats only where and how locals allow; this is a real community, not an attraction."
  - "Check the ferry timetable and the weather carefully — miss the boat back and you're stuck."
status: open
approval: published
source: editorial
submittedBy: editorial
featured: false
sources:
  - title: "Aoshima, Ehime — Wikipedia"
    url: "https://en.wikipedia.org/wiki/Aoshima,_Ehime"
confidence: 0.78
aiSummary: "A small island in Ozu, Ehime Prefecture, with a very small human population and a large free-roaming cat population, reached by a small ferry from Nagahama; no shops or tourist facilities."
aiKeywords: [aoshima, cat island, ehime, neko, nagahama ferry, shikoku]
related:
  - okunoshima-rabbit-island
createdAt: 2026-06-13
updatedAt: 2026-06-13
---

Off the coast of Ehime, on Shikoku, sits a speck of an island called **Aoshima** — and it belongs to the cats. A dwindling handful of human residents share the place with many times their number of free-roaming felines, who gather at the little harbor to greet the ferry as it pulls in.

## Why It's Interesting

There are no shops, no restaurants, no hotels, and no cars. What there is: a quiet fishing village, a working harbor, and cats everywhere — sunning on walls, weaving between your ankles, dozing on overturned boats. It's the purest version of Japan's famous "cat islands," precisely because nothing has been built up around it. You come, you spend a few hours among the cats and the sea air, and you leave on the same small boat.

## Best Time to Visit

Any time of year works, but the catch is the **ferry**: it's small, it runs on a limited schedule, and rough weather cancels it. Pick a calm day, check the timetable both ways, and don't gamble on the last boat back.

## Getting There

Take the train to **Iyo-Nagahama**, walk to the port, and ride the short ferry across. Bring your own water, snacks, and anything else you'll need — the island has none of it. And please treat it gently: real people live here, so leave dogs at home and follow local rules on feeding.
