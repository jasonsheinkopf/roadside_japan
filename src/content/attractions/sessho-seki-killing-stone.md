---
title: "Sessho-seki, the Killing Stone"
summary: "On a steaming sulfur field in Nasu sits a legendary rock said to hold the spirit of a murderous nine-tailed fox — and which famously cracked in two in 2022, prompting half-joking fears the demon escaped."
prefecture: tochigi
city: "Nasu"
address: "Yumoto, Nasu, Nasu District, Tochigi"
lat: 37.1258
lng: 139.9897
category: oddity
tags: [oddity, folklore, volcanic, hidden-gem, history, photography]
seasons: []
months: []
seasonNote: "The barren sulfur field is eerie year-round; autumn colour rings the valley in late October."
difficulty: easy
timeRequired: one-hour
cost:
  type: free
  note: "Free to visit; it's an open-air site above Nasu Yumoto Onsen."
parking:
  available: yes
  note: "Free lot at the Sai-no-Kawara field below Nasu Yumoto."
transit:
  nearestStation: "Kuroiso Station + bus"
  note: "Buses from Kuroiso Station to Nasu Yumoto Onsen, then a short walk up."
accessibility:
  wheelchair: partial
  note: "A boardwalk crosses the field, but the terrain is rough volcanic ground."
dogFriendly: limited
heroImage: "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b8/180726_Sessho-seki_Nasu_Japan05.JPG/960px-180726_Sessho-seki_Nasu_Japan05.JPG"
photos:
  - src: "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b8/180726_Sessho-seki_Nasu_Japan05.JPG/960px-180726_Sessho-seki_Nasu_Japan05.JPG"
    alt: "Sessho-seki, Nasu"
    credit: "Photo via Wikimedia Commons"
    creditUrl: "https://commons.wikimedia.org/wiki/File:180726_Sessho-seki_Nasu_Japan05.JPG"
  - src: "/images/attractions/sessho-seki-killing-stone-1.svg"
    alt: "A large volcanic rock split in two on a steaming sulfur field, a nine-tailed fox spirit looming"
    caption: "The legendary 'killing stone' — it actually cracked open back in 2022. Spooky."
  - src: "/images/attractions/sessho-seki-killing-stone-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel near the split killing stone with a ghostly fox"
    caption: "A fox demon escaped this rock. Foxes are fuzzy. I REALLY don't like him. 😨"
website: "https://www.nasukogen.org/"
tips:
  - "The volcanic gas is real — stay on the boardwalk and don't linger in hollows."
  - "Pair it with a soak in Nasu Yumoto, one of Japan's oldest hot-spring towns, just below."
  - "Read up on Tamamo-no-Mae, the shape-shifting fox of the legend, before you go."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.83
sources:
  - title: "Sessho-seki — Wikipedia"
    url: "https://en.wikipedia.org/wiki/Sessho-seki"
aiSummary: "A volcanic rock on a sulfurous field above Nasu Yumoto Onsen in Tochigi, tied to the legend of a nine-tailed fox spirit; it split into two pieces in March 2022."
aiKeywords: [sessho-seki, killing stone, nine-tailed fox, tamamo no mae, nasu, tochigi]
related:
  - osorezan-bodaiji
createdAt: 2026-06-16
updatedAt: 2026-07-13
cinnamon:
  quote: "The nine-tailed fox's stone split open in 2022. I sniffed it once and left VERY politely."
  emoji: "🦊"
---

Above the old hot-spring town of **Nasu Yumoto**, the forest gives way to a barren, hissing field of yellow sulfur called Sai-no-Kawara — and in the middle of it sits the **Sessho-seki**, the "Killing Stone."

## Why It's Interesting

Legend says the rock holds the vengeful spirit of **Tamamo-no-Mae**, a beautiful courtier who was secretly a **nine-tailed fox demon**; when she was unmasked and slain, her body became this stone, said to kill anything that touched it. The eerie, gas-venting field does nothing to dispel the mood. Then, in **March 2022**, the famous stone was found **split cleanly in two** — weathering, almost certainly, but the internet briefly and gleefully feared the demon had broken free.

## Best Time to Visit

**Year-round.** The bleak sulfur field is most atmospheric under low cloud; **late October** rings the valley in autumn colour.

## Getting There

It's a short walk up from **Nasu Yumoto Onsen**, reachable by bus from **Kuroiso Station** — and the perfect spooky prelude to a hot-spring soak.
