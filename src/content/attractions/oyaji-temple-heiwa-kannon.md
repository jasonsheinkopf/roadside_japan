---
title: "Oya-ji Temple & the Heiwa Kannon"
summary: "Beside the underground Oya quarry, a temple built into a cave shelters Japan's oldest stone Buddhas carved in the cliff — and next door a 27-metre goddess of peace stands hewn straight from the rock face."
prefecture: tochigi
city: "Utsunomiya"
address: "1194 Oyamachi, Utsunomiya, Tochigi"
lat: 36.5928
lng: 139.8167
category: religious
tags: [religious, stone, cliff-carving, history, hidden-gem, photography]
seasons: []
months: []
difficulty: easy
timeRequired: one-hour
cost:
  type: paid
  priceJpy: 500
  note: "Temple admission; the adjacent Heiwa Kannon park is free."
parking:
  available: yes
  note: "Free parking shared with the Oya area sights."
transit:
  nearestStation: "Utsunomiya Station + bus"
  note: "Oya-bound buses from Utsunomiya Station stop nearby."
accessibility:
  wheelchair: partial
  note: "Temple has steps and a cave hall; the Kannon park has a stair climb to the viewpoint."
dogFriendly: limited
heroImage: "/images/attractions/oyaji-temple-heiwa-kannon.svg"
photos:
  - src: "/images/attractions/oyaji-temple-heiwa-kannon-1.svg"
    alt: "A giant Kannon goddess figure carved into a stone cliff face beside a temple"
    caption: "A peace goddess carved straight into the Oya-stone cliff. Massive and calm."
  - src: "/images/attractions/oyaji-temple-heiwa-kannon-2.svg"
    alt: "Mon-chan bowing respectfully before the cliff-carved Kannon statue, peace sign"
    caption: "Carved right into the cliff. I bowed. Twice, just to be safe. 🙏✌️"
website: "https://www.ooyaji.jp/"
tips:
  - "The temple's main hall is built into a rock overhang, sheltering ancient cliff-carved Buddhas."
  - "Climb the steps beside the Heiwa Kannon for a view back over the Oya stone town."
  - "It's a 10-minute walk from the Oya History Museum quarry — do both together."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.82
sources:
  - title: "Oya-ji Temple — official site"
    url: "https://www.ooyaji.jp/"
aiSummary: "A temple in the Oya district of Utsunomiya, Tochigi, built into a rock overhang with ancient cliff-carved Buddhist statues, beside the 27-metre Heiwa (Peace) Kannon carved from the Oya stone cliff."
aiKeywords: [oyaji, oya-ji, heiwa kannon, oya stone, cliff carving, utsunomiya, tochigi]
related:
  - oya-history-museum
createdAt: 2026-06-16
updatedAt: 2026-06-16
---

Right beside the cavernous **Oya stone quarry** outside Utsunomiya, the soft local rock has been carved for worship as well as construction. **Oya-ji Temple** tucks its main hall **into a rock overhang**, and behind it, hewn into the living cliff, are some of **Japan's oldest stone Buddhist carvings**.

## Why It's Interesting

The cave-set hall shelters a set of weather-defying **cliff Buddhas** said to date back over a thousand years — serene faces emerging from the stone itself. Step next door and you meet the **Heiwa Kannon**, a **27-metre goddess of peace** carved directly from the Oya cliff after World War II as a memorial and a prayer. Standing beneath her, dwarfed by a figure that is literally part of the mountain, is genuinely moving.

## Best Time to Visit

**Year-round.** Pair it with the underground quarry next door for a half-day all about **Oya stone**.

## Getting There

Oya-bound buses run from **Utsunomiya Station**; the temple and Kannon are a short walk from the **Oya History Museum** quarry.
