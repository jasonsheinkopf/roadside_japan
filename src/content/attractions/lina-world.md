---
title: "Lina World"
summary: "Yamagata's homegrown amusement park: a friendly, unflashy spread of roller coasters, a big Ferris wheel, a water ride, carousels and character shows — the kind of all-day, low-stress fun that's perfect for families with younger kids."
prefecture: yamagata
city: "Nakayama"
address: "Iwanomori, Nakayama, Higashimurayama District, Yamagata"
lat: 38.3300
lng: 140.2900
category: experience
tags: [amusement-park, kids, family-friendly, rides, experience, summer]
seasons: []
months: []
seasonNote: "Open most of the year; summer adds water-play; check the seasonal calendar."
difficulty: easy
timeRequired: full-day
cost:
  type: paid
  priceJpy: 3500
  note: "Day passport with rides; entry-only tickets are cheaper."
parking:
  available: yes
  note: "Large free parking."
transit:
  nearestStation: "Uzen-Nagasaki Station + shuttle"
  note: "Seasonal shuttle from Uzen-Nagasaki Station; a car is easiest."
accessibility:
  wheelchair: partial
  note: "Mostly flat paved park; some rides have restrictions."
dogFriendly: limited
heroImage: "/images/attractions/lina-world.svg"
photos:
  - src: "/images/attractions/lina-world-1.svg"
    alt: "A cheerful amusement park with a Ferris wheel and a carousel"
    caption: "Yamagata's amusement park — Ferris wheel, coasters, the works. Kid heaven."
  - src: "/images/attractions/lina-world-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel riding a carousel at an amusement park"
    caption: "Cinnamon rode the carousel eleven times yelling 'YEAH!' I supervised from a swan."
website: "https://www.linaworld.co.jp/"
tips:
  - "It's pitched at families with younger kids — gentle rides plus a few real coasters."
  - "Summer adds water-play areas; bring a change of clothes for the little ones."
  - "Get the day passport if you plan to ride a lot; it pays off fast."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.8
sources:
  - title: "Lina World — official site"
    url: "https://www.linaworld.co.jp/"
aiSummary: "Lina World is Yamagata's main amusement park, in Nakayama, with roller coasters, a Ferris wheel, water and gentle rides, and character shows, aimed at families with children."
aiKeywords: [lina world, amusement park, nakayama, yamagata, rides, kids]
related:
  - nasu-highland-park
createdAt: 2026-06-16
updatedAt: 2026-06-16
cinnamon:
  quote: "I rode the kiddie coaster four times. The operator saluted me on the fourth. Honor."
  emoji: "🎢"
  report: |-
    Lina World is Yamagata's own amusement park, and it isn't trying to be anyone else's: a friendly, unflashy spread of coasters, a big Ferris wheel, a water ride, carousels, and character shows, run for local families having an uncomplicated good day.

    I rode the Ferris wheel with a farm family who shared rice crackers at the apex — apex crackers taste better, this is science — and surveyed the Yamagata basin from the top: orchards to the mountains in every direction. The kiddie coaster is, for a squirrel, a legitimately scaled thrill ride, and I logged four laps.

    No hour-long lines, no surge pricing, soft-serve at regular intervals. The humans here are all locals, which tells you what you need to know. Big parks impress; Lina World relaxes. Both are amusement. This one naps after.
---

Every region needs a hometown amusement park, and Yamagata's is **Lina World** — a cheerful, unpretentious spread of rides in the hills near Nakayama that has been making local kids happy for decades.

## Why It's Interesting

It's not the biggest or flashiest park in Japan, and that's exactly the point: it's **relaxed, affordable, and pitched squarely at families with younger children**. There's a big **Ferris wheel**, a couple of genuine **roller coasters**, a log-flume water ride, carousels, gentle kiddie rides, and regular character shows. In **summer** it adds water-play zones — very welcome in the August heat. Lines are short, the staff are kind, and you can do the whole place at a child's pace without stress.

## Best Time to Visit

Open **most of the year**; **summer** is liveliest with water-play. Check the seasonal calendar before going.

## Getting There

Easiest by **car** off the expressway near Nakayama; a seasonal shuttle runs from Uzen-Nagasaki Station.
