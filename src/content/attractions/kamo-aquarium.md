---
title: "Kamo Aquarium (Jellyfish Dream)"
summary: "A white aquarium on the Tsuruoka coast that holds the world's largest jellyfish collection — a Guinness record 50-plus species — climaxing in a five-metre 'Jellyfish Dream' tank where 2,000 moon jellies drift in slow, glowing circles."
prefecture: yamagata
city: "Tsuruoka"
address: "657-1 Imaizumi, Okitama, Tsuruoka, Yamagata"
lat: 38.8175
lng: 139.6300
category: animal
tags: [aquarium, jellyfish, kids, family-friendly, indoor, rainy-day, summer]
seasons: []
months: []
seasonNote: "Indoor and great year-round — a perfect cool escape on a hot August day."
difficulty: easy
timeRequired: half-day
cost:
  type: paid
  priceJpy: 1500
  note: "Adult admission; children much cheaper."
parking:
  available: yes
  note: "Free parking on site."
transit:
  nearestStation: "Tsuruoka Station + bus"
  note: "Bus from Tsuruoka Station to the coast; a car is easiest."
accessibility:
  wheelchair: yes
  note: "Modern, fully step-free building with elevators."
dogFriendly: no
heroImage: "/images/attractions/kamo-aquarium.svg"
photos:
  - src: "/images/attractions/kamo-aquarium-1.svg"
    alt: "Glowing moon jellyfish drifting in a large circular dark aquarium tank"
    caption: "A whole museum of jellyfish, ending in a giant glowing tank. Hypnotic."
  - src: "/images/attractions/kamo-aquarium-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel watching glowing jellyfish in a dark tank"
    caption: "Cinnamon pressed his nose to the glass and whispered 'YEAH!' We stayed an hour."
website: "https://kamo-kurage.jp/"
tips:
  - "Save the giant 'Jellyfish Dream' tank for last — 2,000 moon jellies in a 5-metre drum."
  - "It's fully indoor and air-conditioned: an ideal hot-August stop with kids."
  - "Try the jellyfish ice cream in the café if you're feeling brave."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.86
sources:
  - title: "Kamo Aquarium — Wikipedia"
    url: "https://en.wikipedia.org/wiki/Kamo_Aquarium"
  - title: "Kamo Aquarium — VISIT YAMAGATA"
    url: "https://www.visityamagata.jp/en/spot-turuoka-kamosuizokukan-en/"
aiSummary: "A coastal aquarium in Tsuruoka, Yamagata, holding a Guinness-record collection of over 50 jellyfish species, famous for a five-metre circular tank of 2,000 moon jellyfish."
aiKeywords: [kamo aquarium, jellyfish, kurage, tsuruoka, yamagata, guinness]
related:
  - okunoshima-rabbit-island
createdAt: 2026-06-16
updatedAt: 2026-06-16
cinnamon:
  quote: "A hall of ten thousand glowing jellyfish. I pressed my nose to the glass for one hour."
  emoji: "🪼"
  report: |-
    The Kamo Aquarium on the Tsuruoka coast bet its entire existence on jellyfish, and won: a Guinness-record collection, fifty-plus species, ending in the 'Jellyfish Dream' — a five-metre tank where thousands of moon jellies drift in blue light like slow snow.

    I'll be honest about what happened in front of that tank: nothing. For twenty minutes. Me, motionless, tail slack, thoughts gone. A staff member checked on me. The jellies do that to everyone — the bench in front is full of humans in the same condition, ages three to ninety.

    Smaller tanks upstream show jellies like lace, like lanterns, like bad ideas with tentacles. There's jellyfish ice cream; I performed my duty (fine; texturally confusing). The dream tank at closing time, nearly alone, is the version to chase.
  snapshots:
    - cast: [cinnamon]
      prop: "🪼"
      caption: "Twenty motionless minutes at the Jellyfish Dream tank — thousands of moon jellies falling like slow snow. A staff member checked on me. They do that to everyone."
    - cast: [cinnamon, socks]
      prop: "🍦"
      caption: "Performing our jellyfish ice-cream duty. Verdict: texturally confusing, spiritually necessary. Socks finished his. Slowly."
---

On the coast west of Tsuruoka stands a sleek white building that came back from the brink of closure by betting everything on the most unlikely of stars: **jellyfish**. Today the **Kamo Aquarium** holds the **largest jellyfish collection in the world** — a Guinness-certified **50-plus species** — and it's one of the most quietly mesmerizing places in Tohoku.

## Why It's Interesting

Room after room glows with backlit tanks of drifting, translucent jellies of every size and shape, building to the showstopper: the **"Jellyfish Dream"** tank, a five-metre circular drum in which some **2,000 moon jellyfish** pulse and turn in slow blue light. Kids press their faces to the glass; adults forget to leave. There's also a sea-lion show and a touch pool, but the jellyfish are why the world comes.

## Best Time to Visit

**Year-round**, and especially welcome in **summer** — it's indoor, cool, and calm when the August heat is fierce.

## Getting There

Bus or drive from **Tsuruoka Station** out to the Kamo coast. Pair it with the Shonai beaches or Mt. Haguro.
