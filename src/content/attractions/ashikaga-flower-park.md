---
title: "Ashikaga Flower Park"
summary: "Home to a single 160-year-old wisteria whose violet canopy spreads over a thousand square metres — a spring spectacle so unreal it lights up at night, and whose winter illumination ranks among Japan's best."
prefecture: tochigi
city: "Ashikaga"
address: "607 Hasamacho, Ashikaga, Tochigi"
lat: 36.3146
lng: 139.5200
category: nature
tags: [flowers, wisteria, illumination, scenic, photography, instagrammable, spring]
seasons: [spring, winter]
months: [4, 5]
seasonNote: "The great wisteria peaks mid-April to mid-May; the 'Flower Fantasy' light show runs mid-October to mid-February."
difficulty: easy
timeRequired: half-day
cost:
  type: paid
  note: "Admission is tiered and rises with the bloom; check before you go."
parking:
  available: yes
  note: "Large free parking; a dedicated JR station sits right beside the park."
transit:
  nearestStation: "Ashikaga Flower Park Station"
  note: "Its own JR Ryomo Line station is a 1-minute walk from the gate."
accessibility:
  wheelchair: yes
  note: "Wide, flat, paved paths throughout."
dogFriendly: limited
heroImage: "https://upload.wikimedia.org/wikipedia/commons/thumb/1/10/20190430Ashikaga_Flower_Park_1.jpg/960px-20190430Ashikaga_Flower_Park_1.jpg"
photos:
  - src: "https://upload.wikimedia.org/wikipedia/commons/thumb/1/10/20190430Ashikaga_Flower_Park_1.jpg/960px-20190430Ashikaga_Flower_Park_1.jpg"
    alt: "From Category:Ashikaga Flower Park listing"
    credit: "Photo via Wikimedia Commons"
    creditUrl: "https://commons.wikimedia.org/wiki/File:20190430Ashikaga_Flower_Park_1.jpg"
  - src: "/images/attractions/ashikaga-flower-park-1.svg"
    alt: "A vast 160-year-old wisteria tree dripping violet blossoms over a wide trellis"
    caption: "One wisteria tree, 160 years old, the size of a house. Completely unreal."
  - src: "/images/attractions/ashikaga-flower-park-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel under the giant glowing wisteria at night"
    caption: "A 160-year-old flower waterfall. Cinnamon yelled 'YEAH!' so loud a petal fell. 💜"
website: "https://www.ashikaga.co.jp/"
tips:
  - "The great wisteria's window is short — mid-April to mid-May — and the price tracks the bloom."
  - "Stay after dark for the wisteria illumination; the trellis glows like a violet sky."
  - "Off-season, the 'Flower Fantasy' winter light show is rated one of Japan's top three illuminations."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.85
sources:
  - title: "Ashikaga Flower Park — official site"
    url: "https://www.ashikaga.co.jp/"
  - title: "Great Wisteria of Ashikaga — Japan Today"
    url: "https://japantoday.com/category/features/travel/japan%E2%80%99s-160-year-old-great-wisteria-of-tochigi-is-now-in-full-bloom-ready-for-visitors-2"
aiSummary: "A flower park in Ashikaga, Tochigi, famous for a 160-year-old great wisteria with a canopy over 1,000 square metres, plus a major winter LED illumination."
aiKeywords: [ashikaga flower park, great wisteria, fuji, illumination, ashikaga, tochigi]
related:
  - kawachi-fuji-garden
createdAt: 2026-06-16
updatedAt: 2026-07-13
cinnamon:
  quote: "I stood under the great wisteria and got rained on by purple. 10/10."
  emoji: "💜"
  report: |-
    I found a photo of a purple ceiling on a page of old newspaper lining a chestnut vendor's basket, and spent two days finding out it was real. It's real. It's a single wisteria, 160 years old, whose canopy spreads over a thousand square metres on wires and trellises.

    I climbed nothing. I want that noted. You do not climb a 160-year-old wisteria; you sit under it and let it happen to you. Violet everywhere, bees the size of my head conducting business, the whole thing dripping light. At night they illuminate it and the crowd goes church-quiet.

    The humans queue badly here — go on a weekday, go at opening, or go at night. Loses one acorn for the food stalls selling wisteria ice cream that tastes mostly of purple. Still worth every step.
  snapshots:
    - cast: [cinnamon, socks]
      prop: "💜"
      caption: "Sitting under the 160-year-old wisteria and letting it happen to us. You do not climb a national treasure; I want that noted."
    - cast: [cinnamon]
      prop: "🌙"
      caption: "The night illumination, when the whole crowd goes church-quiet. Come on a weekday, at opening or after dark."
---

Some flowers you admire; the **Great Wisteria of Ashikaga** you stand *beneath*, mouth open. **Ashikaga Flower Park** is built around a single **160-year-old wisteria** whose trained canopy spreads over **more than a thousand square metres**, raining violet blossom from a trellis held up like a sky.

## Why It's Interesting

For a few weeks each spring the park becomes one of the most photographed places in Japan — cascading purple, pink, and white wisteria, including an **80-metre tunnel** of it, all reflected in ponds. Then it gets even more theatrical after dark, when the blooms are **illuminated** into a glowing canopy. And in the off-season the park reinvents itself entirely with **"Flower Fantasy,"** a winter LED light show ranked among **Japan's top three illuminations**.

## Best Time to Visit

**Mid-April to mid-May** for the great wisteria (the ticket price rises with the bloom), and **mid-October to mid-February** for the winter lights.

## Getting There

Absurdly easy: the park has **its own JR station** a one-minute walk from the gate, plus large free parking.
