---
title: "Nasu Onsen Shrine"
summary: "A 1,300-year-old mountain shrine at the top of Nasu Yumoto, wrapped in cedars beside the steaming killing-stone field — guardian of the hot springs, a giant 'lucky' tree, and the samurai archer Nasu no Yoichi."
prefecture: tochigi
city: "Nasu"
address: "Yumoto 182, Nasu, Nasu District, Tochigi"
lat: 37.1235
lng: 139.9940
category: religious
tags: [shrine, religious, history, hidden-gem, autumn-leaves, photography]
seasons: []
months: []
seasonNote: "Atmospheric year-round; autumn color and winter snow both suit the cedars."
difficulty: easy
timeRequired: one-hour
cost:
  type: free
  note: "Free to visit; the grounds adjoin the killing-stone field."
parking:
  available: yes
  note: "Use the Sai-no-Kawara / Nasu Yumoto lots and walk."
transit:
  nearestStation: "Kuroiso Station + bus"
  note: "Bus from Kuroiso to Nasu Yumoto Onsen, then a short uphill walk."
accessibility:
  wheelchair: partial
  note: "Stone steps and forest paths; partly accessible near the entrance."
dogFriendly: limited
heroImage: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/61/Nasu_Yuzen_Shrine_from_Sessho-seki_area.jpg/960px-Nasu_Yuzen_Shrine_from_Sessho-seki_area.jpg"
photos:
  - src: "https://upload.wikimedia.org/wikipedia/commons/thumb/6/61/Nasu_Yuzen_Shrine_from_Sessho-seki_area.jpg/960px-Nasu_Yuzen_Shrine_from_Sessho-seki_area.jpg"
    alt: "Nasu Yuzen shrine seen from the Sessho-seki area"
    credit: "Photo via Wikimedia Commons"
    creditUrl: "https://commons.wikimedia.org/wiki/File:Nasu_Yuzen_Shrine_from_Sessho-seki_area.jpg"
  - src: "/images/attractions/nasu-onsen-shrine-1.svg"
    alt: "An old mountain shrine with a red torii among cedars near steaming sulfur vents"
    caption: "A 1,300-year-old shrine on the volcano, right beside the famous killing stone."
  - src: "/images/attractions/nasu-onsen-shrine-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel bowing at a small red-torii mountain shrine"
    caption: "Prayed for Mon-chan to stay the leader. The shrine made no promises. 🙏"
website: "https://nasu-onsenjinja.jp/"
tips:
  - "It sits right by the Sessho-seki killing stone — do both in one short walk."
  - "Look for the giant 'goyou' lucky cedar and the shrine to archer Nasu no Yoichi."
  - "Autumn cedars and winter snow both make the approach especially atmospheric."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.8
sources:
  - title: "Nasu Onsen Shrine — official site"
    url: "https://nasu-onsenjinja.jp/"
aiSummary: "Nasu Onsen Shrine is a roughly 1,300-year-old shrine at Nasu Yumoto in Tochigi, guardian of the hot springs, beside the Sessho-seki field, associated with a giant sacred cedar and the archer Nasu no Yoichi."
aiKeywords: [nasu onsen shrine, nasu yumoto, sessho-seki, nasu no yoichi, tochigi, shrine]
related:
  - sessho-seki-killing-stone
  - shika-no-yu-onsen
createdAt: 2026-06-16
updatedAt: 2026-07-13
cinnamon:
  quote: "A shrine older than the killing stone legend next door. I bowed extra. Insurance."
  emoji: "⛩️"
---

At the top of **Nasu Yumoto**, where the road meets the steaming sulfur field, an avenue of towering cedars leads up to **Nasu Onsen Shrine** — the roughly **1,300-year-old** guardian of these hot springs, founded, the story goes, when the healing waters were first discovered.

## Why It's Interesting

The setting is half the magic: the shrine sits **right beside the Sessho-seki "killing stone"** and its hissing vents, so a single short walk takes you from folklore and brimstone to mossy, sacred calm. The grounds shelter a **giant sacred cedar**, a small shrine to the legendary archer **Nasu no Yoichi**, and centuries of prayers for safe bathing and good fortune. After the eerie open sulfur field, stepping into the cedar shade feels like crossing into another world.

## Best Time to Visit

**Year-round.** The cedars are striking framed by **autumn color** or dusted with **winter snow**, and quiet most mornings.

## Getting There

Bus from **Kuroiso Station** to **Nasu Yumoto Onsen**, then a short uphill walk — pair it with the killing stone and Shika-no-yu for a compact, atmospheric loop.
