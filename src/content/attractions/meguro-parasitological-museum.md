---
title: "Meguro Parasitological Museum"
summary: "A tiny, free, two-floor museum dedicated entirely to parasites — home to a preserved 8.8-metre tapeworm and a gift shop selling parasite keychains. Tokyo's most gleefully strange date spot."
prefecture: tokyo
city: "Meguro"
address: "4-1-1 Shimomeguro, Meguro, Tokyo"
lat: 35.6258
lng: 139.7066
category: museum
tags: [oddity, museum, indoor, rainy-day, weird, free, transit-accessible, kids]
seasons: []
months: []
difficulty: easy
timeRequired: one-hour
cost:
  type: free
  note: "Free admission; donations welcome."
parking:
  available: no
  note: "Use nearby coin parking; transit is far easier."
transit:
  nearestStation: "Meguro Station (JR / Tokyo Metro) — about 12 min walk"
  note: "Walkable from Meguro Station."
accessibility:
  wheelchair: partial
  note: "Elevator between the two exhibition floors."
dogFriendly: no
heroImage: "/images/attractions/meguro-parasitological-museum.svg"
website: "https://www.kiseichu.org/"
tips:
  - "The 8.8m tapeworm — with a ribbon beside it so you can walk its full length — is the headline exhibit."
  - "The gift shop's parasite-in-resin keychains are a famously odd souvenir."
  - "It's small: an hour is plenty. Pair it with a stroll along the Meguro River."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.88
sources:
  - title: "Meguro Parasitological Museum — official site"
    url: "https://www.kiseichu.org/"
aiSummary: "A small, free museum in Meguro devoted to parasitology, famous for a giant tapeworm specimen and quirky parasite-themed merchandise."
aiKeywords: [parasites, tapeworm, kiseichu, weird museum, tokyo, free museum]
related:
  - oya-history-museum
createdAt: 2025-10-05
updatedAt: 2025-12-01
---

Tucked into a quiet residential block near the Meguro River is a museum so single-minded it has become a legend: the **Meguro Parasitological Museum**, founded in 1953 and devoted entirely to the study of parasites.

## Why It's Interesting

Two compact floors hold some 300 specimens, but everyone comes for one thing — an **8.8-metre tapeworm** removed from a single human host, displayed full-length with a ribbon you can walk alongside to grasp the scale. The tone is scientific, not ghoulish, which somehow makes it more wonderful. The gift shop is half the fun, selling T-shirts and keychains with real parasites suspended in resin.

## Best Time to Visit

Perfect for a rainy day or a hot afternoon — it's small, indoor, air-conditioned, and free. Weekends can get crowded precisely because it's so beloved by the curious.

## Getting There

An easy walk from Meguro Station. There's almost no parking, so come by train and make an afternoon of the riverside neighborhood.
