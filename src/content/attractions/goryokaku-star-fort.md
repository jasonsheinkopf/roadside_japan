---
title: "Goryōkaku Star Fort"
summary: "A Western-style fortress in Hakodate built as a perfect five-pointed star — best seen from the tower above, and unforgettable in spring when 1,600 cherry trees turn the whole star pink."
prefecture: hokkaido
city: "Hakodate"
address: "44 Goryokakucho, Hakodate, Hokkaido"
lat: 41.7969
lng: 140.7569
category: history
tags: [history, star-fort, cherry-blossom, viewpoint, park, spring, photography]
seasons: [spring]
months: [4, 5]
seasonNote: "A great park year-round; the cherry blossoms around the star peak in early May."
difficulty: easy
timeRequired: half-day
cost:
  type: paid
  note: "The park and moat are free; the Goryokaku Tower observation deck is paid."
parking:
  available: limited
  note: "Limited paid parking nearby; trams from Hakodate are easier."
transit:
  nearestStation: "Goryōkaku-kōen-mae tram stop"
  note: "Hakodate tram to Goryōkaku-kōen-mae, then about a 10-minute walk."
accessibility:
  wheelchair: yes
  note: "Flat park paths; the tower has elevators to the observation deck."
dogFriendly: limited
heroImage: "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Goryokaku_Hakodate_Hokkaido_Japan08n.jpg/960px-Goryokaku_Hakodate_Hokkaido_Japan08n.jpg"
photos:
  - src: "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Goryokaku_Hakodate_Hokkaido_Japan08n.jpg/960px-Goryokaku_Hakodate_Hokkaido_Japan08n.jpg"
    alt: "Goryokaku fort, Hakodate"
    credit: "Photo via Wikimedia Commons"
    creditUrl: "https://commons.wikimedia.org/wiki/File:Goryokaku_Hakodate_Hokkaido_Japan08n.jpg"
  - src: "/images/attractions/goryokaku-star-fort-1.svg"
    alt: "An aerial view of the star-shaped moat ringed with pink cherry blossoms"
    caption: "It's a fort shaped like a star. From above it's just showing off."
  - src: "/images/attractions/goryokaku-star-fort-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel on the tower deck above the pink star-shaped fort"
    caption: "A fort shaped like a star! Cinnamon tried to transform into one. Mixed results."
website: "https://www.goryokaku-tower.co.jp/"
tips:
  - "You can only appreciate the star shape from the air — go up the Goryokaku Tower."
  - "Early May is peak cherry blossom; the illuminated evening 'Hoshi-no-Yume' winter lights are lovely too."
  - "Walk the moat and the restored magistrate's office (Bugyosho) inside the walls."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.84
sources:
  - title: "Goryōkaku — Wikipedia"
    url: "https://en.wikipedia.org/wiki/Goryokaku"
aiSummary: "A star-shaped, Western-style fort in Hakodate, Hokkaido, built in the 1860s, now a public park famous for cherry blossoms and best viewed from the adjacent Goryokaku Tower."
aiKeywords: [goryokaku, star fort, hakodate, hokkaido, cherry blossom, bugyosho]
related:
  - takeda-castle-ruins
createdAt: 2026-06-16
updatedAt: 2026-07-13
cinnamon:
  quote: "A fort shaped like a star! Best viewed from the tower, or by being a kite."
  emoji: "⭐"
  report: |-
    From the ground, Goryōkaku is a pleasant park with a moat that keeps turning. From the tower next door, it snaps into shape: a perfect five-pointed star drawn in earth and water, a Western-style fortress from the last gasp of the samurai era.

    I took the tower first (correct order, trust me), pressed my face to the glass, and mapped my climbing route like a general. Then I spent the afternoon down in the star itself — in spring, 1,600 cherry trees fill the ramparts and the whole fortification goes pink, which rather undermines its military dignity and improves everything else.

    The shape only exists from above, which feels like a secret the ground-level humans are cheerfully missing. Tower, then star. Sakura season if you can. History with excellent geometry.
  snapshots:
    - cast: [cinnamon]
      prop: "⭐"
      caption: "Face pressed to the tower glass, mapping the five-pointed star like a general. The shape only exists from above — tower first, always."
    - cast: [cinnamon, mon]
      prop: "🌸"
      caption: "Down in the star itself in spring, 1,600 cherry trees turning the ramparts pink. Undermines the military dignity, improves everything else."
---

From the ground it looks like an ordinary moat-ringed park. From the air it's a flawless **five-pointed star**. **Goryōkaku** was Japan's first Western-style fortress, built in the 1860s on European principles of geometric defense — and its dramatic star shape has long outlived its guns.

## Why It's Interesting

The angular star was designed so defenders could cover every approach with crossfire, a radical idea imported as Japan opened to the world. It saw the last stand of the shogunate's forces in 1869, and is now a beloved public park. The genius twist for visitors: you climb the **Goryōkaku Tower** beside it to look *down* and finally see the whole star — and in spring, that star is outlined by **1,600 cherry trees** in full pink bloom.

## Best Time to Visit

Lovely **year-round**, but **early May** cherry-blossom season is the showstopper, with winter evening illuminations a quieter runner-up.

## Getting There

Take the **Hakodate tram** to Goryōkaku-kōen-mae and walk a few minutes to the park and tower.
