---
title: "Cape Sōya — Japan's Northernmost Point"
summary: "The literal top of Japan: a windswept cape on the Sea of Okhotsk marked by a stark triangular monument, where on clear days you can see the Russian island of Sakhalin across the water."
prefecture: hokkaido
city: "Wakkanai"
address: "Soya Misaki, Wakkanai, Hokkaido"
lat: 45.5231
lng: 141.9367
category: viewpoint
tags: [cape, viewpoint, roadside, monument, scenic, northernmost, photography]
seasons: []
months: []
seasonNote: "Bracingly windy year-round; summer is mildest, winter is brutal but stark and beautiful."
difficulty: easy
timeRequired: one-hour
cost:
  type: free
  note: "Free to visit; just you, the monument, and the wind."
parking:
  available: yes
  note: "Free parking right by the monument."
transit:
  nearestStation: "Wakkanai Station + bus"
  note: "Buses run from Wakkanai Station out to the cape; it's the northern end of the rail network."
accessibility:
  wheelchair: partial
  note: "The monument area is paved and flat, but exposed and very windy."
dogFriendly: yes
heroImage: "https://upload.wikimedia.org/wikipedia/commons/thumb/3/35/The_northernmost_point_of_Japan_monument_in_Soya_cape.jpg/960px-The_northernmost_point_of_Japan_monument_in_Soya_cape.jpg"
photos:
  - src: "https://upload.wikimedia.org/wikipedia/commons/thumb/3/35/The_northernmost_point_of_Japan_monument_in_Soya_cape.jpg/960px-The_northernmost_point_of_Japan_monument_in_Soya_cape.jpg"
    alt: "Northernmost point monument at Cape Soya"
    credit: "Photo via Wikimedia Commons"
    creditUrl: "https://commons.wikimedia.org/wiki/File:The_northernmost_point_of_Japan_monument_in_Soya_cape.jpg"
  - src: "/images/attractions/cape-soya-1.svg"
    alt: "The stark triangular northernmost-point monument against a gray windy sea"
    caption: "The actual top of Japan. There is nowhere further to walk north."
  - src: "/images/attractions/cape-soya-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel at the windy northernmost-point monument"
    caption: "Top of Japan, freezing wind. My cold metal mother would feel right at home. ✌️"
website: "https://www.welcome.wakkanai.hokkaido.jp/"
tips:
  - "On a clear day you can see Sakhalin (Russia) about 43 km across the strait."
  - "It is always windy — bring a layer more than you think, even in summer."
  - "Collect the 'northernmost point' certificate from the nearby shops to prove you made it."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.84
sources:
  - title: "Cape Sōya — Wikipedia"
    url: "https://en.wikipedia.org/wiki/Cape_S%C5%8Dya"
aiSummary: "The northernmost point of mainland Japan, a cape near Wakkanai in Hokkaido marked by a triangular monument, with views toward Sakhalin on clear days."
aiKeywords: [cape soya, soya misaki, northernmost point, wakkanai, hokkaido, okhotsk]
related:
  - aogashima-island
createdAt: 2026-06-16
updatedAt: 2026-07-13
cinnamon:
  quote: "The very top of Japan! Nothing above me but wind and one confused seagull."
  emoji: "🧭"
  report: |-
    Every journal needs a 'top of the country' page, so I rode the coastal bus to Cape Sōya, the northernmost point of Japan, where a stark triangle monument points at the sky and the wind comes straight from Russia without stopping to introduce itself.

    On the clear day I got, Sakhalin sat right there on the horizon — another country, visible from a bus stop. I stood on the monument's tip (four seconds, personal record, wind-adjusted) and squeaked the squeak of geographic completion.

    There's a little shop that sells 'northernmost' everything and a surprising number of very serious motorcyclists having very emotional moments; this is apparently where every rider in Japan finishes something. I get it. I finished something too. The wind kept the receipt.
---

Drive to the very end of the road at the very top of Japan and you arrive at **Cape Sōya**, where the land runs out into the cold Sea of Okhotsk. A stark, pointed monument marks the **northernmost point of mainland Japan** — and on the right day, you can see another country across the water.

## Why It's Interesting

It's a place that's mostly about *arrival*: the satisfying, slightly absurd thrill of standing where you simply cannot go any further north. The triangular monument points at the Pole Star; the wind never stops; and roughly **43 km across the strait** lies the Russian island of **Sakhalin**, visible on clear days as a low gray line. The shops behind you sell "northernmost" everything — ramen, certificates, fuel — and somehow that earnest end-of-the-earth kitsch is exactly the charm.

## Best Time to Visit

**Year-round**, but know what you're signing up for: summer is merely windy and cool; winter is ferociously cold and stark, the sea sometimes choked with drift ice.

## Getting There

Buses run from **Wakkanai**, the northern terminus of Japan's rail network, out to the cape — or drive the lonely coast road there yourself.
