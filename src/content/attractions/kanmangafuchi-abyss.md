---
title: "Kanmangafuchi Abyss"
summary: "A short, mossy gorge walk in Nikko lined with a row of stone Jizo statues in red bibs — the 'Bake Jizo,' famous for the local saying that you can never count them and get the same number twice."
prefecture: tochigi
city: "Nikko"
address: "Kanmangafuchi, Nikko, Tochigi"
lat: 36.7497
lng: 139.5905
category: scenic
tags: [scenic, jizo, nature, hidden-gem, autumn-leaves, photography, history]
seasons: [autumn]
months: [11]
seasonNote: "Peaceful year-round; the gorge is at its loveliest framed by red maples in November."
difficulty: easy
timeRequired: one-hour
cost:
  type: free
  note: "Free to walk; it's an open riverside path."
parking:
  available: limited
  note: "A small lot nearby; many walk from central Nikko."
transit:
  nearestStation: "Nikko Station + bus/walk"
  note: "A bus or pleasant 25-minute walk from the Nikko stations."
accessibility:
  wheelchair: no
  note: "An unpaved riverside path with roots, stones, and uneven footing."
dogFriendly: limited
heroImage: "/images/attractions/kanmangafuchi-abyss.svg"
photos:
  - src: "/images/attractions/kanmangafuchi-abyss-1.svg"
    alt: "A row of mossy red-bibbed stone Jizo statues along a forested river gorge"
    caption: "A line of mossy Jizo by a rushing river. Count them — you never get the same number."
  - src: "/images/attractions/kanmangafuchi-abyss-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel quietly passing the row of red-bibbed Jizo by the river"
    caption: "We counted the little guardians — a different number each time. Cinnamon found acorns. 🍁"
website: "https://www.nikko-kankou.org/"
tips:
  - "Try to count the Jizo on the way in and out — locals say the totals never match."
  - "Pair it with the Tamozawa Imperial Villa garden, a short walk away."
  - "Go in November for the maples, or early morning any season for solitude."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.82
sources:
  - title: "Kanmangafuchi Abyss — Nikko tourism"
    url: "https://www.nikko-kankou.org/"
aiSummary: "A scenic riverside gorge in Nikko, Tochigi, lined with a row of stone Jizo statues known as Bake Jizo, said to defy being counted, especially beautiful in autumn."
aiKeywords: [kanmangafuchi, bake jizo, jizo, nikko, gorge, tochigi]
related:
  - okunoin-koyasan
createdAt: 2026-06-16
updatedAt: 2026-06-16
---

Away from Nikko's grand shrines, a quiet path follows the **Daiya River** into a small basalt gorge formed by an old lava flow. This is the **Kanmangafuchi Abyss**, and its real draw stands in a long, silent row along the bank.

## Why It's Interesting

Dozens of weathered stone **Jizo statues** — guardians of travelers and children — sit shoulder to shoulder in faded **red bibs and caps**, mossed over and watching the water. They're nicknamed the **Bake Jizo** ("ghost Jizo") because of a gentle local legend: count them on your way in and again on your way out, and you'll never arrive at the same number. True or not, it makes the walk quietly magical, especially with the river rushing and maples flaming overhead.

## Best Time to Visit

**Year-round**, but **November** frames the statues in red autumn leaves; early mornings are hushed and empty.

## Getting There

A bus or a lovely **25-minute walk** from the Nikko stations — a peaceful counterpoint to the area's busier sights.
