---
title: "Enoshima Island"
summary: "A small tidal island off Kanagawa reached on foot over a causeway, packed onto one hill: a sea-dragon shrine, candlelit sea caves, a beachside lighthouse tower, and sweeping Sagami Bay views — all in a single easy loop."
prefecture: kanagawa
city: "Fujisawa"
address: "1-9 Enoshima, Fujisawa, Kanagawa"
lat: 35.2989
lng: 139.4803
category: scenic
tags: [scenic, nature, viewpoint, religious, outdoor, kids, family-friendly, photography]
seasons: [summer, autumn]
months: [9]
seasonNote: "September keeps summer's warmth and clear sea air without the peak-August beach crowds."
difficulty: moderate
timeRequired: half-day
cost:
  type: paid
  note: "Individual sights (Iwaya Caves ¥500, Sea Candle ¥800) are separately ticketed; an Enopass combo ticket covers escalators, the shrine, and the Sea Candle."
parking:
  available: limited
  note: "Paid lots near the causeway fill quickly on weekends; the train is easier."
transit:
  nearestStation: "Enoshima Station (Enoden Line) or Katase-Enoshima Station (Odakyu Line)"
  note: "About 60-70 minutes from central Tokyo via JR to Fujisawa + Enoden, or via Odakyu from Shinjuku."
accessibility:
  wheelchair: partial
  note: "Paid escalators ('Enoshima Escar') climb much of the hill, but the caves and lower shrine paths involve stairs."
dogFriendly: limited
website: "https://www.fujisawa-kanko.jp/en/"
tips:
  - "The Iwaya Caves at the island's far tip are candlelit sea caves you can walk into, tied to local dragon-god legends — a genuinely different feel from the shrine and shopping streets above."
  - "Climb the Sea Candle lighthouse-tower for a 360-degree view of Sagami Bay; on a clear day Mt. Fuji is visible across the water."
  - "The paid Enoshima Escar outdoor escalators save most of the uphill walk to Enoshima Shrine — worth it in late-summer heat even with the ticket cost."
heroImage: "https://upload.wikimedia.org/wikipedia/commons/thumb/4/4e/Enoshima_Sea_Candle.jpg/960px-Enoshima_Sea_Candle.jpg"
photos:
  - src: "https://upload.wikimedia.org/wikipedia/commons/thumb/4/4e/Enoshima_Sea_Candle.jpg/960px-Enoshima_Sea_Candle.jpg"
    alt: "Enoshima Sea Candle tower"
    credit: "Photo via Wikimedia Commons"
    creditUrl: "https://commons.wikimedia.org/wiki/File:Enoshima_Sea_Candle.jpg"
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.83
sources:
  - title: "Enoshima — Travel Japan (JNTO)"
    url: "https://www.japan.travel/en/spot/2096/"
  - title: "Enoshima Iwaya Caves — Tokyo Cheapo"
    url: "https://tokyocheapo.com/place/iwaya-caves/"
aiSummary: "Enoshima, a small tidal island off Fujisawa in Kanagawa, combines a dragon-god shrine, candlelit sea caves, and a lighthouse-tower viewpoint over Sagami Bay in one compact, walkable loop about an hour from Tokyo."
aiKeywords: [enoshima, enoshima island, iwaya caves, sea candle, fujisawa kanagawa]
featured: false
createdAt: 2026-07-12
updatedAt: 2026-07-13
cinnamon:
  quote: "Shrines, a lighthouse, and dragon legends on one island. I did it all for a snack."
  emoji: "🗼"
  report: |-
    Enoshima is what happens when Japan fits a whole vacation onto one hill you can walk to over a causeway: sea-dragon shrine, candlelit caves, lighthouse garden, and Sagami Bay glittering in every direction.

    I took the escalators up (yes, the island has escalators; no, I will not be taking questions about whether I rode the handrail) and paid respects to Benzaiten, the goddess who — the legend says — married the local five-headed dragon to make it stop misbehaving. The sea caves at the far end drip and echo, and you carry a little candle lantern through them like a very short pilgrim.

    Go on a clear winter day and Fuji floats over the bay. The kites here steal food from human hands — protect your snacks; they show no honor.
---

Barely a kilometer around, Enoshima crams a surprising amount into one small tidal island connected to the mainland by a footbridge across Katase River's mouth.

## Why It's Interesting

The island climbs from a shopping-street base up to Enoshima Shrine, dedicated to Benzaiten, a goddess linked to a local dragon-taming legend, before descending the far side toward the Iwaya Caves — two sea caves carved by centuries of wave action, walked with a hand-held candle rather than electric lighting, and tied to the same dragon-god mythology. Partway up, the Sea Candle, a lighthouse-shaped observation tower, gives a 360-degree view over Sagami Bay, with Mt. Fuji visible on clear days. It's dense enough to feel like a real half-day excursion despite being barely an hour from central Tokyo.

## Best Time to Visit

September holds onto summer warmth and clear coastal air while the peak-August beach crowds thin out — a good window for the uphill walking without the July-August heat and density.

## Getting There

About 60-70 minutes from central Tokyo: JR to Fujisawa then the Enoden Line to Enoshima Station, or the Odakyu Line direct from Shinjuku to Katase-Enoshima Station.
