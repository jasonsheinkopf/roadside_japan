---
title: "Fujishiro Seiji Museum"
summary: "A theater-like museum devoted to Seiji Fujishiro's luminous 'kage-e' shadow art — jewel-colored cut-paper fairytale worlds — reached through a garden where a cat sculpture quietly shows you the way."
prefecture: tochigi
city: "Nasu"
address: "Takaku-otsu, Nasu, Nasu District, Tochigi"
lat: 37.0100
lng: 140.0420
category: art
tags: [art, museum, shadow-art, kirie, indoor, rainy-day, hidden-gem]
seasons: []
months: []
difficulty: easy
timeRequired: one-hour
cost:
  type: paid
  priceJpy: 1300
  note: "Adult admission to the galleries and garden."
parking:
  available: yes
  note: "Free parking on site."
transit:
  nearestStation: "Nasushiobara Station + bus/taxi"
  note: "Bus or taxi from Nasushiobara Station into the highland museum area."
accessibility:
  wheelchair: partial
  note: "Mostly step-free galleries; the garden path has some gravel."
dogFriendly: no
heroImage: "/images/attractions/fujishiro-seiji-museum.svg"
photos:
  - src: "/images/attractions/fujishiro-seiji-museum-1.svg"
    alt: "A glowing backlit shadow-art fairytale scene in jewel colors"
    caption: "Cut-paper shadow art, backlit and glowing like stained-glass dreams. Magical."
  - src: "/images/attractions/fujishiro-seiji-museum-2.svg"
    alt: "Mon-chan eyeing a cat statue guide while Cinnamon the squirrel admires the shadow art"
    caption: "A CAT statue guides you here. A cat. As a leader. I do not like this precedent."
website: "https://www.f-seiji.jp/"
tips:
  - "The works are backlit 'kage-e' shadow pictures — the dim, theatrical lighting is the point."
  - "Follow the garden's cat sculpture from the gate to the brick stained-glass chapel."
  - "It's indoor and dreamy — a perfect rainy-day pairing with the nearby museums."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.82
sources:
  - title: "Fujishiro Seiji Museum (Art of Nasu Highland)"
    url: "https://www.f-seiji.jp/"
aiSummary: "A museum in the Nasu highlands dedicated to artist Seiji Fujishiro's backlit cut-paper 'kage-e' shadow art, presented theatrically, with a garden, a guiding cat sculpture, and a stained-glass chapel."
aiKeywords: [fujishiro seiji, kage-e, shadow art, kirie, nasu, tochigi, museum]
related:
  - nasu-stained-glass-museum
createdAt: 2026-06-16
updatedAt: 2026-06-16
cinnamon:
  quote: "Shadow art that glows like candy. I floated out of there, frankly."
  emoji: "🌈"
  report: |-
    I followed a cat statue's gaze through a garden in Nasu — always follow a cat statue's gaze, they place those deliberately — and found the Fujishiro Seiji Museum, a theater of glowing shadow art.

    Fujishiro's 'kage-e' are cut-paper worlds lit from behind: jewel-colored fairytale scenes with silhouetted children, foxes, and cathedrals of light. It sounds gentle and it is, but stand in the dark hall where the big panoramas glow and it goes right past gentle into quietly overwhelming. He worked into his nineties. It shows — the late rooms are the brightest.

    The humans here whisper without being asked. Water lilies float in the entrance pools. Loses one acorn only because the gift shop postcards cannot glow, an unfair standard, but I hold it.
---

Of all the Nasu highland museums, the **Fujishiro Seiji Museum** is the most quietly enchanting. It's devoted to **Seiji Fujishiro**, a master of **kage-e** — backlit cut-paper "shadow pictures" — whose fairytale scenes glow with the saturated color of stained glass.

## Why It's Interesting

The museum is staged like a **theater**: you walk through dim halls where Fujishiro's intricate cut-paper worlds are illuminated from behind, each one a luminous storybook tableau of forests, castles, and creatures. A **garden** leads you in past a long gatehouse and a small **brick chapel** of real stained glass, with a charming **cat sculpture** acting as your unofficial guide. It rewards slowing down — people happily lose an hour here.

## Best Time to Visit

**Year-round** — indoor, calm, and weatherproof, at its best when you have time to let your eyes adjust to the glow.

## Getting There

In the **Nasu museum belt** above Nasushiobara Station, simplest by car as one stop on a gallery-hopping afternoon.
