---
title: "Showa Shinzan"
summary: "A steaming red lava dome that pushed up out of a flat wheat field in the 1940s — Japan's youngest mountain, born in wartime and lovingly documented by the local postmaster who watched it grow."
prefecture: hokkaido
city: "Sobetsu"
address: "Showashinzan, Sobetsu, Usu District, Hokkaido"
lat: 42.5419
lng: 140.8590
category: nature
tags: [volcano, geology, steam, oddity, viewpoint, hidden-gem, photography]
seasons: []
months: []
seasonNote: "Steams year-round; the red rock against fresh snow in winter is especially striking."
difficulty: easy
timeRequired: one-hour
cost:
  type: free
  note: "Viewing the dome is free; the nearby Usuzan ropeway and museum are paid."
parking:
  available: yes
  note: "Paid lots shared with the Usuzan ropeway base."
transit:
  nearestStation: "Toya Station + bus"
  note: "Buses from Toya Station toward Lake Toya / Usuzan; a car is easiest."
accessibility:
  wheelchair: partial
  note: "You view it from a flat paved area; you cannot climb the protected dome."
dogFriendly: limited
heroImage: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/02/Mount_Showa-shinzan_and_touyako.JPG/960px-Mount_Showa-shinzan_and_touyako.JPG"
photos:
  - src: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/02/Mount_Showa-shinzan_and_touyako.JPG/960px-Mount_Showa-shinzan_and_touyako.JPG"
    alt: "Showa Shinzan with Lake Toya"
    credit: "Photo via Wikimedia Commons"
    creditUrl: "https://commons.wikimedia.org/wiki/File:Mount_Showa-shinzan_and_touyako.JPG"
  - src: "/images/attractions/showa-shinzan-1.svg"
    alt: "A steaming reddish-brown lava dome rising from flat ground with vents smoking"
    caption: "This whole mountain is younger than my grandpa. Wild."
  - src: "/images/attractions/showa-shinzan-2.svg"
    alt: "Mon-chan and Cinnamon the squirrel before the steaming red lava-dome volcano"
    caption: "A mountain that grew out of a field. Cinnamon yelled 'YEAH!' at it. Fair enough. 🌋"
website: "https://www.toya-usu-geopark.org/"
tips:
  - "It's still hot and growing slowly — the red colour is baked rock, and it really does steam."
  - "Ride the adjacent Usuzan ropeway for views over the crater and Lake Toya."
  - "Read the story of postmaster Masao Mimatsu, who charted the eruption to protect it from wartime secrecy."
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.82
sources:
  - title: "Shōwa-shinzan — Wikipedia"
    url: "https://en.wikipedia.org/wiki/Sh%C5%8Dwa-shinzan"
aiSummary: "A lava dome volcano near Lake Toya in Hokkaido that emerged from farmland between 1943 and 1945, still steaming, famous for being recorded by local postmaster Masao Mimatsu."
aiKeywords: [showa shinzan, lava dome, lake toya, usu, hokkaido, mimatsu, new volcano]
related:
  - noboribetsu-jigokudani
createdAt: 2026-06-16
updatedAt: 2026-07-13
cinnamon:
  quote: "A volcano that grew out of a wheat field in two years, while the postmaster took notes. Heroic nerd."
  emoji: "🌋"
  report: |-
    In 1943, a Hokkaido wheat field started rising, and by 1945 it was a steaming red lava dome — Showa Shinzan, Japan's newest mountain, born in wartime secrecy while the local postmaster, Masao Mimatsu, measured its growth against a string because the government was too busy to document a whole new mountain.

    His pencil-drawn growth charts (the 'Mimatsu Diagram') became internationally famous science, and his statue now faces the mountain he adopted. I sat at the statue's feet and looked where he looks: at a rust-red cone still steaming faintly, in a field that used to be flat.

    The Usuzan ropeway next door gives you the dome, the lake, and the 2000 eruption scars in one ride. Mountains are usually older than everybody. Here, the grandmothers are older than the mountain. I love it here.
  snapshots:
    - cast: [cinnamon]
      prop: "🌋"
      caption: "At the postmaster's statue's feet, looking where he looks: the rust-red cone that rose from a wheat field between 1943 and 1945, still steaming faintly."
    - cast: [cinnamon, socks]
      prop: "📏"
      caption: "Honoring the Mimatsu Diagram: he measured a newborn mountain's growth against a string because the government was too busy. Socks calls him the patron saint of documentation."
---

Most mountains are unimaginably old. **Showa Shinzan** is not. Between **1943 and 1945**, in the middle of a flat wheat field near Lake Toya, the ground bulged, cracked, and heaved upward into a steaming red dome of lava — and people stood there and watched it happen.

## Why It's Interesting

It is, in effect, **Japan's newest mountain**, still warm and faintly smoking decades later, its rock baked a rusty red. The story is as good as the geology: with wartime authorities suppressing news of the eruption, the local **postmaster, Masao Mimatsu**, meticulously recorded the dome's daily growth from his window — measurements scientists still use. He later bought the land to protect it. You can't climb it (it's protected and hot), but you can stand at its foot, watch it steam, and ride the nearby ropeway for the bigger picture.

## Best Time to Visit

**Year-round.** The contrast of the red dome and its white steam against **winter snow** is especially dramatic.

## Getting There

It shares a base area with the **Usuzan ropeway** near **Lake Toya**, reachable by bus from Toya Station or, more easily, by car on a Lake Toya loop.
