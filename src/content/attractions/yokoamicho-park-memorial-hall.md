---
title: "Yokoamicho Park & Tokyo Memorial Hall"
summary: "A quiet park built over the site where roughly 44,000 people died in a single firestorm during the 1923 Great Kanto Earthquake — now a memorial hall holding the ashes of that disaster's victims and, later, of the 1945 Tokyo firebombing dead."
prefecture: tokyo
city: "Ryogoku"
address: "2-3-25 Yokoami, Sumida, Tokyo"
lat: 35.6971
lng: 139.7958
category: history
tags: [history, free, oddity]
seasons: []
months: [9]
seasonNote: "A memorial service is held every September 1st, the anniversary of the 1923 earthquake."
difficulty: easy
timeRequired: one-hour
cost:
  type: free
parking:
  available: no
transit:
  nearestStation: "Ryogoku Station (JR Sobu / Toei Oedo lines) — about 5 min walk"
  note: "Adjacent to Kyu-Yasuda Teien and the Japanese Sword Museum."
accessibility:
  wheelchair: partial
dogFriendly: unknown
tips:
  - "This exact ground was a former army clothing depot where as many as 44,000 people sheltering from the 1923 earthquake's fires were killed by a sudden firestorm — one of the single deadliest events in the city's history."
  - "The Memorial Hall (Ireido) holds the ashes of roughly 58,000 earthquake victims and, later, 105,000 victims of the March 1945 Tokyo firebombing — a genuinely somber stop, not a casual photo-op."
  - "The adjoining Reconstruction Memorial Hall (built 1931) displays objects distorted and melted by the fires — heat-fused glass bottles, twisted metal — as evidence of the disaster's intensity."
heroImage: "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a2/Tokyo_Memorial_Hall.jpg/960px-Tokyo_Memorial_Hall.jpg"
photos:
  - src: "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a2/Tokyo_Memorial_Hall.jpg/960px-Tokyo_Memorial_Hall.jpg"
    alt: "Tokyo Memorial Hall in Yokoamicho Park"
    credit: "Photo via Wikimedia Commons"
    creditUrl: "https://commons.wikimedia.org/wiki/File:Tokyo_Memorial_Hall.jpg"
status: open
approval: published
source: ai-agent
submittedBy: "ai:research-agent"
confidence: 0.85
sources:
  - title: "Yokoamicho Park — Tokyo's Memorial for 2 Tragedies of the 20th Century"
    url: "https://www.kanpai-japan.com/tokyo/yokoamicho-park"
  - title: "Yokoamichō Park — Wikipedia"
    url: "https://en.wikipedia.org/wiki/Yokoamich%C5%8D_Park"
aiSummary: "A Sumida Ward park and memorial hall marking the site of a catastrophic 1923 earthquake firestorm, now housing the ashes of both Great Kanto Earthquake and 1945 Tokyo firebombing victims."
aiKeywords: [yokoamicho park, great kanto earthquake memorial, tokyo firebombing memorial, ireido]
featured: false
createdAt: 2026-07-04
updatedAt: 2026-07-13
cinnamon:
  quote: "A quiet garden that remembers Tokyo's hardest days. I sat with the pigeons and was thankful."
  emoji: "🕊️"
  report: |-
    Yokoamicho Park is the quietest place in Sumida and carries the heaviest load: on this spot, in the 1923 earthquake's firestorm, roughly 44,000 people died in a single night. The Memorial Hall holds their ashes, and later, the ashes of the 1945 air-raid dead joined them — one small park keeping Tokyo's two worst nights.

    I came the way I hope visitors do: unhurried, in the morning, with nothing else planned after. The hall's paintings and the small museum tell it plainly; the flower stands outside are never empty; the pigeons conduct themselves, somehow, appropriately.

    It's ten minutes from the sumo arena and the museums, and it belongs on the same walk — the neighborhood's joy and its memory hold hands here. Bow, learn, sit under the ginkgos a while. Some parks are parks. This one is a promise.
---

A green, unassuming park a few minutes from the sumo stadium that turns out to sit on one of the heaviest pieces of ground in Tokyo.

## Why It's Interesting

On September 1, 1923, the Great Kanto Earthquake struck at midday, just as people across the city were cooking lunch over open flames; the fires that followed merged into a firestorm. Tens of thousands of people had gathered on this exact plot — then a former army clothing depot, seen as an open, safe space — and an estimated 44,000 of them were killed here when the firestorm swept through. The Memorial Hall (Ireido) built afterward holds the ashes of roughly 58,000 earthquake victims citywide, and was later expanded to also hold the remains of 105,000 people killed in the March 1945 U.S. firebombing of Tokyo. It's an unusual, weighty kind of stop for a Saturday walk — a direct, physical link to two of the deadliest events in the city's modern history, and a quietly resonant one given the fire-safety education center waiting at the end of the route.

## Best Time to Visit

Any time — it's outdoor and free, though a memorial service each September 1st draws visitors specifically for the anniversary.

## Getting There

About five minutes from Ryogoku Station, sharing a corner with Kyu-Yasuda Teien and the Japanese Sword Museum.
