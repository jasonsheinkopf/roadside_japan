---
title: "Takachiho Gorge"
summary: "A narrow basalt canyon of columnar cliffs and a waterfall plunging straight into emerald water — row a small boat to its base, through a landscape woven into Japan's oldest creation myths."
prefecture: miyazaki
city: "Takachiho"
address: "Mukoyama, Takachiho, Nishiusuki District, Miyazaki"
lat: 32.6957
lng: 131.3033
category: scenic
tags: [scenic, nature, photography, hidden-gem, outdoor, autumn-leaves, romantic]
seasons: [spring, summer, autumn]
months: [11]
seasonNote: "Lush and cool in summer; maple-framed in November."
difficulty: moderate
timeRequired: half-day
cost:
  type: paid
  note: "Gorge walk is free; rowboat rental is paid and time-limited."
parking:
  available: yes
  note: "Several paid lots; the closest fill first."
transit:
  note: "No train. Reached by bus from Nobeoka/Kumamoto or by car; remote."
accessibility:
  wheelchair: partial
  note: "Upper viewpoint paths are partly accessible; the boats and lower path are not."
dogFriendly: limited
website: "https://takachiho-kanko.info/"
tips:
  - "Book the rowboat slot online in advance — peak-season waits can run hours."
  - "Visit the nearby Takachiho Shrine in the evening for the nightly kagura dance."
  - "Heavy rain can suspend the boats; check before you drive out."
status: open
approval: published
source: editorial
submittedBy: editorial
featured: false
aiSummary: "A dramatic columnar-basalt gorge in Miyazaki with a famous waterfall and rowboats, steeped in Japanese mythology."
aiKeywords: [takachiho, manai falls, gorge, kagura, basalt columns, kyushu myth]
related:
  - ibusuki-sand-bath
createdAt: 2025-05-12
updatedAt: 2025-10-22
---

In the mountainous heart of Kyushu, the Gokase River has sliced a slot through ancient volcanic rock to create **Takachiho Gorge** — walls of perfectly geometric basalt columns rising over impossibly green water.

## Why It's Interesting

The signature image is the **Manai Falls** spilling 17 metres directly into the gorge, with a tiny rowboat drifting beneath it. You can be the one in that boat: rentals let you paddle right to the base of the falls. This is also mythic ground — Takachiho is where, in legend, the sun goddess Amaterasu hid in a cave and plunged the world into darkness, and the region keeps those stories alive in nightly **kagura** dances.

## Best Time to Visit

Summer is cool and green inside the gorge; November frames the cliffs in red maple. Mornings are calmer and better for photos and boat availability.

## Getting There

There's no train — most visitors come by car or by bus from Kumamoto or Nobeoka. The remoteness is part of why it still feels sacred.
